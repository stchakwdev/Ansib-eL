...
class DecisionLineage:
    ...
    reasoning: ReasoningRecord | None
...
class TrustScorer:
    ...
    def _calculate_weight(
        self, change_size: int, complexity: ChangeComplexity, review_time_ms: int
    ) -> float:
    ...
    def _apply_time_decay(self, current_score: float, last_activity: datetime) -> float:
        ...
        return float(max(decayed_score, min(current_score, 0.1)))
    ...
    def record_decision(
        ...
        with self._lock, self._get_connection() as conn:
            ...
            logger.info(
                f"Recorded {decision.value} for agent {agent_id} on commit {commit_hash[:8]}"
            )
...
class LineageTracker:
    ...
    def record_reasoning(
        ...
        library_choices: dict[str, str] | None = None,
    ) -> ReasoningRecord:
        """
    ...
    def get_lineage(self, commit_hash: str) -> DecisionLineage | None:
        """
        ...
        with self._get_connection() as conn:
            ...
            agent_id = (
                UUID(reasoning_row["agent_id"])
                if reasoning_row
                else (UUID(decision_row["agent_id"]) if decision_row else uuid4())
            )
    ...
    def trace_decision_chain(self, start_commit: str, max_depth: int = 10) -> list[DecisionLineage]:
        ...
        """
        chain: list[DecisionLineage] = []
        current: str | None = start_commit
        visited = set()
    ...
    def search_reasoning(
        self, keyword: str, agent_id: UUID | None = None, limit: int = 50
...
class TrustLineageManager:
    ...
    def record_decision(self, **kwargs) -> DecisionRecord:
        """Convenience delegate to TrustScorer.record_decision."""
        return self.trust.record_decision(**kwargs)

    def get_trust_score(self, agent_id: UUID) -> TrustScore:
        """Convenience delegate to TrustScorer.get_trust_score."""
        return self.trust.get_trust_score(agent_id)
    ...
    def should_auto_approve(self, agent_id: UUID, change_size: int) -> bool:
        """Convenience delegate to TrustScorer.should_auto_approve."""
        return self.trust.should_auto_approve(agent_id, change_size)

    def get_agent_profile(self, agent_id: UUID):
        """Convenience delegate to TrustScorer.get_agent_profile."""
        return self.trust.get_agent_profile(agent_id)

    def get_all_agents(self):
        """Convenience delegate to TrustScorer.get_all_agents."""
        return self.trust.get_all_agents()
    ...
    def record_complete_decision(
        ...
        """
        # Record decision first (creates agent profile if needed)
        ...
        # Record reasoning (agent profile must exist for FK constraint)
    ...
    def get_decision_with_lineage(self, commit_hash: str) -> DecisionLineage | None:

...
class TrustScorer:
    ...
    def _update_agent_profile(self, conn: sqlite3.Connection, agent_id: UUID,
        ...
        if row:
            ...
            last_activity = datetime.fromisoformat(row["last_activity"]) if row["last_activity"] else datetime.now(timezone.utc)
            ...
            cursor.execute(
                ...
                (new_score, new_confidence, new_sample_count, new_tier.value,
                 new_avg_time, datetime.now(timezone.utc).isoformat(), datetime.now(timezone.utc).isoformat(),
                 str(agent_id))
            )
        else:
            ...
            cursor.execute(
                """INSERT INTO agent_profiles 
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (str(agent_id), f"agent_{str(agent_id)[:8]}", datetime.now(timezone.utc).isoformat(),
                 new_score, new_confidence, 1, new_tier.value, 0, review_time_ms,

...
if HAS_PYDANTIC:
    ...
    class AgentPydanticModel(BaseModel):
        ...
        parent_task_id: str | None = Field(
            None, description="Parent task ID if spawned from another task"
        )
        ...
        def validate_timestamp(cls, v: str) -> str:
            try:
                ...
                raise ValueError(f"Invalid ISO timestamp: {v}") from err
            return v
...
class Agent:
    ...
    metadata: AgentMetadata | None = field(default=None)
...
class AgentContext:
    ...
    def __init__(self, agent_id: UUID, base_workspace_path: str | None = None):
        """
...
class AgentManager:
    ...
    def get_agent_by_string_id(self, agent_id_str: str) -> Agent | None:
        """
    ...
    def get_agent_context(self, agent_id: UUID) -> AgentContext | None:
        """
    ...
    def _load_agents(self) -> None:
        ...
        try:
            ...
            # Load agents
            for _agent_id_str, agent_data in data.get("agents", {}).items():
                agent = Agent.from_dict(agent_data)
                self.agents[agent.agent_id] = agent

            # Load task mappings
            for task_id, agent_ids in data.get("agents_by_task", {}).items():
                self._agents_by_task[task_id] = {UUID(aid) for aid in agent_ids}
    ...
    def cleanup_terminated(self, max_age_hours: int | None = None) -> int:
        """
...
class AgentCommunicationBus:
    ...
    def __init__(self):
        ...
        self._subscribers: dict[UUID, list[Callable[..., Any]]] = {}
    ...
    def subscribe(self, agent_id: UUID, callback: Callable[..., Any]) -> None:
        """Subscribe an agent to receive messages."""
        if agent_id not in self._subscribers:
            self._subscribers[agent_id] = []
        self._subscribers[agent_id].append(callback)

    def unsubscribe(self, agent_id: UUID, callback: Callable[..., Any]) -> None:
    ...
    def get_messages_for_agent(
        self, agent_id: UUID, message_type: str | None = None
    ) -> list[AgentMessage]:
        """Get messages for a specific agent."""
        messages = [m for m in self._messages if m.recipient_id == agent_id]
    ...
    def clear_messages(self, agent_id: UUID | None = None) -> None:
        """Clear messages, optionally for a specific agent."""
        if agent_id:
            self._messages = [m for m in self._messages if m.recipient_id != agent_id]
        else:

...
from ansibel.git_wrapper import AgentMetadata, GitWrapper
...
if CLICK_AVAILABLE:
    ...
    def merge(branch_name: str, strategy: str, target: str | None, yes: bool):
        ...
        if not yes and not click.confirm("Proceed with merge?"):
    ...
    @click.option("--json-output", "-j", is_flag=True, help="Output as JSON")
    def history(limit: int, agent: str | None, json_output: bool):
            ...
            for branch in branches:
                ...
                if agent_id:
                    ...
                    trust_color = (
                        Colors.GREEN
                        if trust > 0.7
                        else (Colors.YELLOW if trust > 0.4 else Colors.RED)
                    )
    ...
    def review(branch: str | None):
        ...
        for idx, branch_info in enumerate(branches_to_review, 1):
            ...
            action = click.prompt(
                "Action",
                type=click.Choice(
                    ["merge", "skip", "diff", "details", "quit"], case_sensitive=False
                ),
                default="details",
            )
    ...
    def diff(branch_a: str, branch_b: str | None):
...
def create_argparse_cli():
    ...
    agents_parser.add_argument(
        "--status",
        "-s",
        choices=["active", "merged", "closed", "all"],
        default="all",
        help="Filter by status",
    )
...
def run_argparse_cli():
    ...
    # Metadata command
        ...
        metadata = wrapper.get_commit_metadata(args.commit_hash)  # type: ignore[assignment]
